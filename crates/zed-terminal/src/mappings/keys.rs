...
pub fn to_esc_str(
    ...
    let manual_esc_str: Option<&'static str> = match (keystroke.key.as_ref(), &modifiers) {
        ...
        ("D", AlacModifiers::Alt)
            if !mode.contains(TermMode::APP_CURSOR) && keystroke.key_char.is_none() =>
        {
            Some("\x1bb")
        }
        ("S", AlacModifiers::Alt)
        ...
        ("right", AlacModifiers::Alt) if !mode.contains(TermMode::APP_CURSOR) => Some("\x1bf"),
        ("left", AlacModifiers::Alt) if !mode.contains(TermMode::APP_CURSOR) => Some("\x1bb"),
...
mod tests {
    ...
    fn alt_arrow_word_motion_matches_readline_expectations() {
        ...
        // Keep modified arrow behavior in application cursor mode.
        assert_eq!(
